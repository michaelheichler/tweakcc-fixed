...
const PATCH_DEFINITIONS = [
  ...
  },
  {
    ...
    group: PatchGroup.FEATURES,
    description:
      "[EXPERIMENTAL] Route Claude Code's Bash search through fff (fast file finder). CC 2.1.186 shadows the shell `grep`→embedded ugrep and `find`→embedded bfs (and offers `rg`); the agent uses grep ~136x more than rg. This repoints all three at a per-platform fff wrapper: literal, tool-equivalent regex, case-insensitive (-i), and context (-A/-B/-C) content searches → fff (relevance-ranked, typo-tolerant, warm-index daemon); anything fff can't reproduce byte-for-byte — PCRE, multiline/newline- or empty-matching regex, only-matching (-o), single-file, non-recursive grep, non-ASCII, lines over 512 bytes, piped stdin → re-exec the real embedded ugrep/bfs/ripgrep. Every engine still ships; nothing is removed; any uncertainty falls back. Transparent (no prompt-compliance reliance) and CC-scoped (the user's own terminal grep/find/rg are untouched). Installs the wrapper into ~/.tweakcc/fff.",
  },
  {
    id: 'dream-mode',
    name: 'Dream mode',
