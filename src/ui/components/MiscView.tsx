  ...
  const items: MiscItem[] = useMemo(
    () => [
      ...
      },
      {
        ...
        description:
          "Route Claude Code's Bash search through fff. CC shadows the shell grep->embedded ugrep and find->embedded bfs (and offers rg); the agent uses grep far more than rg. This repoints all three at a per-platform fff wrapper: literal, tool-equivalent regex, case-insensitive (-i), and context (-A/-B/-C) content searches -> fff (relevance-ranked, typo-tolerant, warm-index daemon); anything fff can't reproduce byte-for-byte (PCRE, multiline/newline/empty regex, only-matching -o, single-file, non-recursive grep, non-ASCII, lines over 512 bytes, piped stdin) -> re-exec the real embedded ugrep/bfs/ripgrep. Every engine still ships; any uncertainty falls back. Transparent (no prompt-compliance reliance), CC-scoped (your own terminal grep/find/rg are untouched). Installs the wrapper into ~/.tweakcc/fff.",
